...
begin
  if AnsiUpperCase(edtTipo.Text) = 'FÍSICA' then
  begin
    lbCPF.Caption := 'CPF';
...
begin
  ...
  begin
    Application.MessageBox('ATENÇÃO!: INSCRIÇÃO ESTADUAL - INVÁLIDO',
      'Preenchimento do Formulário', +MB_OK);
...
begin
  ...
  if not ValidarUF(edtUF.Text) then
  begin
    MostraAviso('Consulte o seu CEP ou faça cadastro da sua CIDADE');
    begin
      lbect1.Visible := True;
      Exit;
    end;
...
begin
  ...
  begin
    ...
    begin
      ShowMessage('É necessário digitar Captcha');
      edtCaptcha.SetFocus;
    end;
  end;
...
begin
  ...
  //  Quando Usuário digitar CEP, vai fazer uma Validação
...

{
  ...
  //  Chamo uma Função/Metodo para buscar Registros/Cadastros pelo CEP
  //  E jogará as informações do Registros/Cadastrados pelo CEP
  ACBrCEP.BuscarPorCEP(edtCEP.Text);
  edtNumEnd.SetFocus;
  ...
  //  Se tiver  mais de um endereço, Mostra tela  pro usuário selecionar
