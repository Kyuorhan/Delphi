...
begin
  ...
  if edtNatureza.KeyValue = 'E' then
  begin
  ...
  // - Não permitir fazer movimentos em caixa que não esteja aberto
  if  (dmPrincipal.qTemp.FieldByName('situacao').AsAnsiString = 'F') then
  begin
    MostraAviso('Ja existe um caixa fechado no dia: '
  ...
  begin
    ...
    MostraMensagem('Este caixa já está Inativo!');
    Exit;
  end;
  ...
  begin
    MostraAviso('A Data do Lançamento deve ser Inserido!');
    edtData.SetFocus;
    Exit;
  end;
...
begin
  ...
  dmCaixa.sqlCaixas_MovimentosSituacao.AsString   := dmCaixa.sqlCaixas_AberturasSituacao.AsString;
end;
